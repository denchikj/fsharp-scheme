...
let rec equalFn =
    ...
    | [ LispList a; LispList b ] ->
        let ret =
            a.Length = b.Length && List.zip a b |> List.forall (fun (x, y) -> equal2 x y)
    ...
    | [ a; b ] ->
        ...
        let ret = anyUnpackerEqual || (eqv [ a; b ] |> (=) (Result.Ok(LispBool true)))
...
let makeVarArgs = makeFunc << Some << (fun v -> v.ToString())
...
let runIOSafe<'T> (action: unit -> 'T) =
    try
        action () |> Result.Ok
    with e ->
        e.ToString() |> DefaultError |> throwError
...
let rec readProc =
    ...
    | [ LispPort(ReadHandle reader) ] -> runIOSafe (fun () -> reader.ReadLine()) |> Result.map LispString
...
let rec writeProc =
    ...
    | [ obj; LispPort(WriteHandle writer) ] ->
        runIOSafe (fun () -> writer.Write(sprintf "%s" (obj.ToString())))
        |> Result.map (fun _ -> LispBool true)
    | args -> BadSpecialForm("readProc", LispList args) |> throwError

let readFile filename =
    runIOSafe (fun () -> System.IO.File.ReadAllText(filename))
...
let load filename =
    readFile filename |> Result.bind readExprList
...
let rec eval (env: Env) =
    ...
    | LispList(LispAtom "lambda" :: LispList p :: body) -> makeNormalFunc env p body
    | LispList(LispAtom "lambda" :: LispDottedList(LispAtom var :: args, LispAtom vargs) :: body) ->
        makeVarArgs vargs env args body |> Result.bind (defineVar env var)
    | LispList(LispAtom "lambda" :: (LispAtom vargs) :: body) -> makeVarArgs vargs env [] body
